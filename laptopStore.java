...
public class laptopStore {
    public static void main(String[] args) {
        Set<Laptop> laptops = new HashSet<>();      // создается колекция set и в нее добавляем экземпляры класса laptop 
        ...
        Map<Integer, String> criteria = new HashMap<>(); // создается колекция map (словарь) и в нее добавляем критерии для поиска в колекции ноутбеков
        criteria.put(1, "ОЗУ");
        criteria.put(2, "Объем ЖД");
        criteria.put(3, "Операционная система");
        criteria.put(4, "Цвет");

        Scanner scanner = new Scanner(System.in); // создается сканер ввода и map для установки фильтра(ов) 
        Map<String, Object> filters = new HashMap<>();

        boolean continueFiltering = true; // установка флага для цикла while 

        while (continueFiltering) { // while для ввода критериев и установки фильтров
            ...
            switch (criterion) { // оператор свич заменяет множественный if-elseif 
                case 1:
    ...
    public static void filterLaptops(Set<Laptop> laptops, Map<String, Object> filters) { // применение фильтра и отображение результата
